    ...
    def __init__(self, opt, name, run_id, data_dict, job_type='Training'):
        ...
        if isinstance(opt.resume, str):
            ...
            self.wandb_run = wandb.init(config=opt, resume="allow",
                                        ...
                                        name=name, job_type=job_type, id=run_id) if not wandb.run else wandb.run
        if self.wandb_run:
        ...
        if isinstance(opt.resume, str):
            ...
            if modeldir:
                ...
                opt.weights, opt.save_period, opt.batch_size, opt.bbox_interval, opt.epochs, opt.hyp = \
                    str(self.weights), config.save_period, config.total_batch_size, \
                    config.bbox_interval, config.epochs, config.opt['hyp']

...
#                         print(len(f.lms), f.euler)
                    ...
                    if int((use_phone_frame+8) * 21 / 30 * self.fps) >= self.frame_3s:
                        result['result']['category'] =  3 #3
                        break

                    elif int((mouth_open_frame+8) * 21 / 30 * self.fps) >= self.frame_3s:
                        result['result']['category'] = 2
                        break

                    elif int((look_around_frame+8) * 21 / 30 * self.fps) >= self.frame_3s:
                        result['result']['category'] = 4
                        break

                    elif int((eyes_closed_frame+8) * 21 / 30 * self.fps) >= self.frame_3s:

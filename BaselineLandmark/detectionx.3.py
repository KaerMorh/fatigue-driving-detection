...
def run_video(video_path, save_path):
    ...
    tickness_yolo = int(fps / 1)
    fps_3s = int(fps / tickness) * 3#
    fps_3s_yolo = int(fps / tickness_yolo) * 3#
    alpha = 1#9帧中取7帧
    ...
    while cap.isOpened():
        ...
        if cnt < int(0.2*fps): continue
        if (cnt-int(0.2*fps)) % tickness != 0 and not (cnt in last_confirm):
            run_tracker = False
        else:
            run_tracker = True

        if (cnt-int(0.2*fps)) % tickness_yolo != 0:
            run_yolo = False
        else:
            run_yolo = True

        if not (run_yolo or run_tracker):
            continue

        if cnt + int(0 * fps) > frames:  # 最后三秒，若没有异常则不判断了
            #如果result_list最后一位是0
            if len(result_list) > 0 and len(result_yolo_list) > 0 and result_list[-1] == 0 and result_yolo_list[-1] == 0 :
                break
        ...
        if run_yolo:
            result_yolo_cnt_list.append(cnt)
            ...
            result_yolo_list.append(frame_result)
        ...
        if use_phone_frame >= 3:  #帧数
            result['result']['category'] = 3
            break

        elif look_around_frame >= 3:  #帧数
            result['result']['category'] = 4
            break

        elif mouth_open_frame >= 5:#帧数
            result['result']['category'] = 2
            break

        elif eyes_closed_frame >= 6:  #帧数
    ...
    print(f'len(yolo_list):{len(result_yolo_list)}')
    print(f'result_yolo_list:{result_yolo_list}')
    print(f'result_yolo_cnt_list:{result_yolo_cnt_list}')
